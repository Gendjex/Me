...
def NOTIFIED_ACCEPT_GROUP_INVITATION(op):
    ...
    try:
        ...
        sendMessage(op.param1,"Budayakan Baca Note !!! yah Ka 😊\nSemoga Betah Kk 😘")
    except Exception as e:
        print e
...
def NOTIFIED_KICKOUT_FROM_GROUP(op):
	...
	sendMessage(op.param1, client.getContact(op.param3).displayName + "Bye! Jangan baper yaa Hahaha 􀜁􀅔Har Har􏿿")
    except Exception as e:
        print e
...
def NOTIFIED_LEAVE_GROUP(op):
    try:
        sendMessage(op.param1, client.getContact(op.param2).displayName + " Bye bye  􀜁􀅔Har Har􏿿")
    except Exception as e:
        print e
...
#-----------------------------------------------------------------------------------------------------------------------------------
		...
		if msg.text == "sp":
		    ...
		    sendMessage(msg.to, "Waktu sekarang = " + datetime.datetime.today().strftime('%H:%M:%S'))
                if msg.text == "gift":
                    sendMessage(msg.to, text="gift sent", contentMetadata={'PRDID': '3b92ccf5-54d3-4765-848f-c9ffdc1da020',
                                    'PRDTYPE': 'THEME',
